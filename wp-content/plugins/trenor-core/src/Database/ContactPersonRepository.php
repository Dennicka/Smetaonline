...
final class ContactPersonRepository extends BaseRepository
{
    ...
    public function byClient(int $clientId): array
    {
        ...
        return $wpdb->get_results(
            $wpdb->prepare("SELECT * FROM {$this->table()} WHERE client_id = %d ORDER BY is_primary DESC, id DESC", $clientId),
            ARRAY_A
        ) ?: [];
    }
    ...
    public function byProperty(int $propertyId): array
    {
        ...
        return $wpdb->get_results(
            $wpdb->prepare("SELECT * FROM {$this->table()} WHERE property_id = %d ORDER BY is_primary DESC, id DESC", $propertyId),
            ARRAY_A
        ) ?: [];
    }
    ...
    public function byProject(int $projectId): array
    {
        ...
        return $wpdb->get_results(
            $wpdb->prepare("SELECT * FROM {$this->table()} WHERE project_id = %d ORDER BY is_primary DESC, id DESC", $projectId),
            ARRAY_A
        ) ?: [];
    }
}
